...
static u32 leer_lados_desde_input(Grafo G, struct LadoSt *lados_temp) {
    // Lectura de los lados y almacenamiento en el arreglo auxiliar. Su complejidad es O(m)
    for (u32 i = 0; i < G->m; i++) {
        ...
        // Se guarda la ubicacion que tendrá en lados_temp_aux el indice del vertice v2 del lado
...
// Función auxiliar para comparar dos lados. Se usa en qsort y da un orden ascendente comparando v1
// y luego comparando v2 en caso de empate
...
static void asignar_nombre_y_grado(Grafo G, struct LadoSt *lados_temp, u32 *lados_temp_aux) {
    // Se recorre el arreglo auxiliar y se asigna el nombre del vertice y el grado de cada vertice.
    // Su complejidad es O(m)
    u32 j = 0;
    for (u32 i = 0; i < G->n; i++) {
        // Se asigna el nombre del vertice en el arreglo de vertices y se inicializa el grado en 0
        G->vertices[i].nombre = lados_temp[j].v1;
        G->vertices[i].grado = 0;

        // Se recorre el arreglo auxiliar hasta que se encuentre un lado que no tenga el mismo v1
        // que el vertice actual (Como están ordenados por v1, cuando se encuentre un lado con un v1
        // distinto siginifica que ya se recorrieron todos los lados que tienen el vertice actual)
        while (j < 2 * G->m && lados_temp[j].v1 == G->vertices[i].nombre) {
            // Se incrementa el grado del vertice actual
            G->vertices[i].grado++;
            // Se guarda el indice del vertice v1 del lado en el arreglo auxiliar (En la ubicación
            // que leerá el lado inverso para saber la posición de su v2)
        ...
        // Se actualiza el valor de delta si el grado del vertice actual es mayor que el actual
        // valor de delta
        if (G->vertices[i].grado > G->delta)
            G->delta = G->vertices[i].grado;
    }
}

static void asignar_vecinos(Grafo G, struct LadoSt *lados_temp, u32 *lados_temp_aux) {
    // Se recorre el arreglo de vertices y se asignan los vecinos de cada vertice. Su complejidad es
    // O(m)
    u32 j = 0;
    for (u32 i = 0; i < G->n; i++) {
        // Se asigna la dirección del primer vecino del vertice actual en el arreglo de
        // lados_vecinos
        ...
        // distinto siginifica que ya se recorrieron todos los lados que tienen el vertice actual)
        while (j < 2 * G->m && lados_temp[j].v1 == G->vertices[i].nombre) {
            // Se asigna el indice del vertice v2 del lado en el arreglo de lados_vecinos
...
Grafo ConstruirGrafo() {
    ...
    // Arreglo auxiliar para almacenar los lados y arreglo auxiliar para luego asignar los indices
    // de los lados vecinos
    ...
    // Ordenamiento de los lados a partir de v1. Su complejidad se supone que es O(m log m)
    // dependiendo de la implementación de qsort
...
u32 Grado(u32 i, Grafo G) { return NumeroDeVertices(G) > i ? G->vertices[i].grado : MAX_U32; }

u32 IndiceVecino(u32 j, u32 i, Grafo G) {
    return NumeroDeVertices(G) > i && Grado(i, G) > j ? G->vertices[i].vecinos[j] : MAX_U32;
}
